...
Replace the `public/images/perfil.jpeg` file with your own avatar image. It is used on the `/about` page as well as on the dynamically generated `open-graph` images.

## Favicon

Replace the `src/app/favicon.svg` file with your own favicon.
...
Social links are rendered on the `/about` page and in the footer based on the array configured in the `social` object. You can set custom icons to each, but don't forget to import them in `src/once-ui/icons.ts`. Read the Rasgo documentation](https://once-ui.com/docs/icons) for more information.
